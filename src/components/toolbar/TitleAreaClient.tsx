...
export default function TitleAreaClient() {
  ...
  const [ titVal, setTitVal ] = useState(title);
    ...
    <header className="fixed left-0 top-0 flex gap-3 h-12 w-full items-center justify-between border-b border-slate-200 bg-white px-4">
      <div className="flex min-w-0 items-center gap-3">
        <input
          type="text"
          value={titVal}
          onChange={(event) => setTitVal(event.target.value)}
          onBlur={() => titVal.trim() === "" && setTitVal(title) }
        ...
        />
        {(title.trim() !== titVal.trim() && titVal.trim() !== "")
          ? <button 
              onClick={saveTitle}
      ...
      >
        ...
        <p>이 앱은 간단하게 태블릿 환경에서 도안을 그리기 위해 만들어졌기 때문에, PC 환경에서 크고 자세한 대바늘 기호 도안은 위 엑셀 파일을 이용하여 만드는 것을 추천드립니다.</p>
      </Modal>
    </header>
  );
}
