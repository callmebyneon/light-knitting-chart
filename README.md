<h1 align="center">
  <img src="./src/app/apple-icon.png"></img><br />
...
뜨개 차트와 컬러 차트를 만들 때 범용 그래픽 도구는 코/단 단위의 그리드, 뜨개 기호, 레이어별 참조 이미지 관리, 오른쪽 아래 기준 번호 표시 같은 도메인 기능을 직접 지원하지 않습니다. 혹은 복잡하고 편리한 도메인 기능을 지원하는 앱은 대게 사용량 제한이 있습니다. 이 앱은 별도의 DB 사용 등을 포기하고 대신 뜨개 차트 작성에 필요한 최소 기능을 한 화면에서 빠르게 사용할 수 있도록 만드는 것을 목표로 합니다.
...
- 레이어는 드래그 앤 드롭(PC only) 혹은 컨텍스트 메뉴에서 순서를 바꿀 수 있습니다.
- 레이어 컨텍스트 메뉴에서 투명도, 이름 변경, 삭제, 복제, 병합, 이미지 크기 조정을 수행합니다.
- 편집 상태와 히스토리는 sessionStorage에 자동 저장됩니다.
